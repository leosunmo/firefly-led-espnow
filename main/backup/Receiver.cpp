...
int Receiver::parseESPNOWData(const uint8_t *data, uint16_t data_len, const uint8_t *src_addr, Message *message) {
    ...
    switch (payloadType) {
        ...
        case PayloadType::RegistrationSuccessful:
            expectedPayloadSize = 0; // RegistrationSuccessful has no additional payload
            break;
    ...
    // Only validate the payload size for non-zero expected payloads
    // For empty payloads like RegistrationSuccessful or Keepalive, allow any data_len >= sizeof(MessageData)
    if (expectedPayloadSize > 0 && data_len < sizeof(MessageData) + expectedPayloadSize) {
        ESP_LOGE(TAG, "Data length is insufficient for payload type: %d", static_cast<int>(payloadType));
        return -1;
    }

    // Extract the received CRC value for comparison later
    uint16_t receivedCrc = rawMessage->crc;
    ...
    if (payloadType == PayloadType::RegistrationSuccessful) {
        ESP_LOGI(TAG, "CRC Debug: data_len=%d, expectedPayloadSize=%zu", data_len, expectedPayloadSize);
        ESP_LOGI(TAG, "Received CRC: 0x%04X", receivedCrc);

        // Manually log the contents of messageCopy
        ESP_LOGI(TAG, "Message Copy (with zeroed CRC):");
        for (size_t i = 0; i < data_len; ++i) {
            ESP_LOGI(TAG, "Byte %zu: 0x%02X", i, reinterpret_cast<const uint8_t *>(messageCopy)[i]);
        }
        
        ESP_LOGI(TAG, "Raw Message:");
        for (size_t i = 0; i < data_len; ++i) {
            ESP_LOGI(TAG, "Byte %zu: 0x%02X", i, reinterpret_cast<const uint8_t *>(rawMessage)[i]);
        }
    }
    ...
    if (calculatedCrc != receivedCrc) {
        ESP_LOGE(TAG, "CRC mismatch: calculated %04X, received %04X", calculatedCrc, receivedCrc);
        
        // Debug the CRC calculation more thoroughly for any message type with issues
        ESP_LOGI(TAG, "CRC calculation debug:");
        uint16_t crc = UINT16_MAX; // Starting value
        for (size_t i = 0; i < data_len; ++i) {
            uint8_t byte = reinterpret_cast<const uint8_t *>(messageCopy)[i];
            crc = esp_crc16_le(crc, &byte, 1);
            ESP_LOGI(TAG, "After byte %zu (0x%02X): CRC = 0x%04X", i, byte, crc);
        }
            
        // Reject all messages with CRC mismatches
        return -1;
    }
    ...
    // For RegistrationSuccessful messages, always accept regardless of sequence number
    if (payloadType == PayloadType::RegistrationSuccessful) {
        ESP_LOGI(TAG, "Accepting RegistrationSuccessful message with sequence number: %d", rawMessage->seq_num);
    ...
    switch (message->payload_type) {
        ...
        case PayloadType::RegistrationSuccessful: {
            // RegistrationSuccessful has an empty payload structure
            // It's okay to have zero payload size since the struct is empty
            ESP_LOGI(TAG, "Received RegistrationSuccessful message");
            RegistrationSuccessfulPayload payload;
            message->parsed_payload = payload;
            break;
        }
        case PayloadType::Keepalive: {
            // Keepalive has an empty payload structure
            // It's okay to have minimal or zero payload size
            ESP_LOGD(TAG, "Received Keepalive message");
            break;
        }
        default:
...
void Receiver::checkKeepalive(void *pvParameter) {
    ...
    while (true) {
        ...
        if (isRegistered && (currentTime - lastKeepaliveTime > 10000)) { // 10-second timeout
            ...
            // Clear sequence numbers when restarting registration
            // We might want to clear only the specific peer's sequence number, but for now, we clear all
